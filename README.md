...
If an older version reported **Display slot 2 already contains an overlay**, update and retry setup. Companion now works around occupied overlay slots rather than requiring every slot from 2–10 to be empty. If fewer than two slots are available, the error lists the occupied slots and directs you to **Check setup → Copy setup report** for support. An occupied overlay is separate from the keyboard's pending-settings flag.

Five occupied slots in XPANEL do not by themselves fill Companion's supported range. If slots 6–10 are empty, setup uses them automatically. The app reserves slot 1 and uses slots 2–10; ten is the app's validated range, not a claimed hardware limit. It does not write to unverified additional slots. If all supported slots are occupied, two empty slots are needed; if one is already available, one more is needed. The current XPANEL overlay page has no clear/delete control, so the error directs users to support instead of giving an unavailable removal step. Existing overlays are never automatically erased to make room.
