...
class DisplayCache{
 constructor(directory,render){this.directory=directory;this.render=render;this.som=null;this.manifest=null;this.lastSlot=null;this.updating=false;this.watching=false;this.config=null;this.caps=false;this.layer=false;this.desired='idle';this.cache=new Map();this.data={};this.lastFrame='';this.error='';this.selectionAt=0;}
 ...
 async write(slot,selected,caps,layer=false){const config=selected==='idle'?this.config:{...this.config,indicator:{...this.config.indicator,enabled:false}};const svg=this.render(selected,config,{caps,layer},this.data),png=await sharp(Buffer.from(svg)).png().toBuffer();await this.som.uploadOverlay(slot,png);this.lastSlot=slot;this.selectionAt=Date.now();this.cache.set(this.key(selected,caps,layer),slot);return svg;}
 ...
 async refreshLive(){if(!this.som||!this.config||this.updating||this.watching||this.layer||this.desired!=='idle')return;const svg=this.render('idle',this.config,{caps:this.caps,layer:false},this.data);if(svg===this.lastFrame)return;const used=[this.cache.get('idle:0'),this.cache.get('idle:1')];const target=[2,3,10].find(s=>!used.includes(s));if(!target)return;this.updating=true;const caps=this.caps;
  try{await this.write(target,'idle',caps);this.lastFrame=svg;}catch(e){this.error=e.message;throw e;}finally{this.updating=false;this.displayReady();}
 }
