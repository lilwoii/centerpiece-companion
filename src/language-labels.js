(function(root){
 ...
 function languageLabel(label,language){if(!language||!Array.isArray(language.labels)||language.id==='qwerty')return label;return String(label).split('+').map(part=>{const i=names.indexOf(part);return i>=0?language.labels[i]||part:part;}).join('+');}
