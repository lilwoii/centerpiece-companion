class Startup {
 ...
 read(){if(!this.app.isPackaged)return{available:false,enabled:false};const value=this.app.getLoginItemSettings({...this.options,path:'"'+this.options.path+'"'});return{available:true,enabled:!!value.openAtLogin&&value.executableWillLaunchAtLogin!==false};}
