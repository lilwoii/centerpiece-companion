...
function configuredSVG(selected,config,locks,data={}){
 ...
 const metric=reading(plugin?.id,data);if(metric)return `<g transform="translate(1605 ${15+index*86})"><rect width="80" height="78" rx="9" fill="${index===selection?'#48607a':'#243040'}" stroke="${index===selection?'#9bb9ff':'#374457'}" stroke-width="3"/>${metric.icon?`<g transform="translate(30 2) scale(.6)">${weatherIcon(metric.icon)}</g>`:''}<text x="40" y="47" text-anchor="middle" fill="#edf2fa" font-family="Arial" font-size="${metric.value.length>6?17:24}" font-weight="bold">${escape(metric.value)}</text><text x="40" y="67" text-anchor="middle" fill="#b8c8dc" font-family="Arial" font-size="9">${escape(metric.label)}</text></g>`;
 ...
 const layer=(locks.layer||data.language?.id&&data.language.id!=='qwerty')?layout.keys.map((key,index)=>{if(locks.layer&&index!==26)return '';const x=((key.x||0)+key.width/2)*1920/1800,y=((key.y||0)+key.height/2)*550/500;const color=config.layerColor||i.on;return `${!locks.layer?`<rect x="${x-key.width*.43}" y="${y-17}" width="${key.width*.86}" height="34" rx="4" fill="#111820" fill-opacity=".94"/>`:''}${locks.layer&&index===26?`<rect x="${x-44}" y="${y-40}" width="88" height="80" rx="8" fill="${color}" fill-opacity=".65" stroke="${color}" stroke-width="3"/>`:''}<text x="${x}" y="${y+7}" text-anchor="middle" font-family="Arial" font-size="${index===26?30:18}" font-weight="bold" fill="${locks.layer&&index===26?'#111820':'#edf2fa'}">${escape(languageLabel(data.baseLabels?.[index]||physical[index]||'',data.language).slice(0,9))}</text>`;}).join(''):'';
 return `<svg xmlns="http://www.w3.org/2000/svg" width="1920" height="550"><g><rect x="1597" y="5" width="96" height="397" rx="12" fill="#111820" fill-opacity=".96" stroke="#48607a"/>${rows}<text x="1645" y="374" text-anchor="middle" fill="#edf2fa" font-family="Arial" font-size="10">PLUGINS</text><text x="1645" y="391" text-anchor="middle" fill="#b8c8dc" font-family="Arial" font-size="12">L1 + P</text></g>${i.enabled?`<rect x="${i.x}" y="${i.y}" width="${i.width}" height="${i.height}" rx="8" fill="${locks.caps?i.on:i.off}" fill-opacity=".48" stroke="${locks.caps?i.on:i.off}" stroke-width="4"/>`:''}${layer}${detailSVG(config,data)}</svg>`;
}
