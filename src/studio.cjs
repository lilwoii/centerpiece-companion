...
class Studio {
  constructor() {
    ...
    this.handle = new HID.HID(device.path); this.pending = null; this.sequence = 0; this.frame = []; this.inFrame = false; this.escape = false;
      ...
      for (const byte of raw.subarray(3, 3 + raw[1] - 2)) {
        if (this.escape) { this.frame.push(byte); this.escape = false; }
          ...
          try { const response = Response.toObject(Response.decode(Buffer.from(this.frame)), {defaults:false});
            if (response.requestResponse?.requestId === this.pending?.id) this.finish(null, response.requestResponse);
  ...
  request(body) {
    ...
    return new Promise((resolve,reject)=>{
      this.pending={id,resolve,reject,timer:setTimeout(()=>this.finish(new Error('Keyboard configuration query timed out')),4000)};
