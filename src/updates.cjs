...
class Updates extends EventEmitter{
 ...
 start(){if(!/^[A-Za-z0-9-]+$/.test(config.githubOwner)||!/^[A-Za-z0-9_.-]+$/.test(config.githubRepo))return;
  ...
  const {NsisUpdater}=require('electron-updater');const cacheRoot=require('./update-storage.cjs').updateStorage(this.app);const u=new NsisUpdater();Object.defineProperty(u.app,'baseCachePath',{get:()=>cacheRoot});this.cacheRoot=cacheRoot;this.updater=u;u.autoDownload=false;u.autoInstallOnAppQuit=false;u.allowDowngrade=false;u.allowPrerelease=this.app.getVersion().includes('-');u.logger=null;
 ...
 install(){if(!this.updater||this.state.status!=='ready')throw Error('Download the update first.');const temp=require('./update-storage.cjs').prepareUpdateTemp(this.cacheRoot);process.env.TEMP=temp;process.env.TMP=temp;this.updater.quitAndInstall(false,true);}
}
