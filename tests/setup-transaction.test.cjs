...
test('XPANEL reset after verified installation repairs both stock shortcuts and preserves every unrelated key',async t=>{const {x,directory,deps}=fixture(t);await startSetup(directory,{includeWidget:true},deps);const backup=JSON.parse(fs.readFileSync(path.join(directory,'hardware.json')));x.map.layers[1].bindings[26]={behaviorId:60498};x.map.layers[1].bindings[55]={behaviorId:56811};x.map.layers[0].bindings[4]={behaviorId:50397,param1:0x70009};const before=structuredClone(x.map);x.edits=[];const result=await startSetup(directory,{includeWidget:true},deps);assert.equal(result.needsConfirmation,false);assert.deepEqual(x.edits.map(e=>e.keyPosition),[26,55]);before.layers[1].bindings[26]=pluginBinding;before.layers[1].bindings[55]=widgetBinding;assert.ok(sameKeymap(before,x.map));assert.deepEqual(JSON.parse(fs.readFileSync(path.join(directory,'hardware.json'))).originalBinding,backup.originalBinding);});
test('only slash reverted after setup is repaired even when plugin shortcut is still correct',async t=>{const {x,directory,deps}=fixture(t);await startSetup(directory,{includeWidget:true},deps);x.map.layers[1].bindings[55]={behaviorId:56811};x.edits=[];await startSetup(directory,{includeWidget:true},deps);assert.deepEqual(x.edits.map(e=>e.keyPosition),[55]);});
test('a custom shortcut after verified setup remains untouched',async t=>{const {x,directory,deps}=fixture(t);await startSetup(directory,{includeWidget:true},deps);x.map.layers[1].bindings[55]={behaviorId:50397,param1:0x70004};x.edits=[];const saves=x.saves;await assert.rejects(startSetup(directory,{includeWidget:true},deps),{code:'KEYBOARD_WIDGET_KEY_CUSTOMIZED',writesAttempted:false});assert.equal(x.edits.length,0);assert.equal(x.saves,saves);});
