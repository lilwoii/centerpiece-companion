...
test('widget cycling wraps and rapid taps save every selection but coalesce display work',async()=>{assert.equal(nextWidget('off'),'spotify');const workspace={config:{widget:{type:'spotify',text:'saved text',x:1710}},save(c){this.config=c;}};let applied=0,changed=0;const cycle=new WidgetCycle(workspace,async()=>{applied++;},()=>changed++);for(let i=0;i<3;i++)cycle.press();assert.equal(workspace.config.widget.type,'cpu');assert.equal(workspace.config.widget.text,'saved text');assert.equal(changed,3);await new Promise(r=>setTimeout(r,240));assert.equal(applied,1);cycle.close();assert.equal(new Set(types).size,12);assert.equal(types.includes('follow'),false);assert.equal(nextWidget('text'),'off');assert.equal(nextWidget('follow'),'spotify');});
